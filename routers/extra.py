...
from crud import create_skill, add_skill_to_user, get_user, get_user_skills, create_job_application, get_applications_for_user, send_message, get_conversation_messages, create_conversation, add_admin_monitor, delete_user_skill
...
@router.delete("/users/{user_id}/skills/{skill_id}", status_code=204)
def delete_skill(user_id: int, skill_id: int, session: Session = Depends(get_session)):
    """Elimina una habilidad del usuario"""
    user = get_user(session, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="Usuario no encontrado")

    success = delete_user_skill(session, user_id, skill_id)
    if not success:
        raise HTTPException(
            status_code=404, detail="Habilidad no encontrada para este usuario")
...
def user_report(user_id: int, session: Session = Depends(get_session), current_user=Depends(get_current_user)):
    ...
    activities = session.exec(select(ActivityLog).where(
        ActivityLog.user_id == user_id).order_by(desc(ActivityLog.created_at)).limit(20)).all()
    # messages count
    messages_count = session.exec(select(Message).where(
